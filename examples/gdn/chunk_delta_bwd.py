...
def tilelang_chunk_gated_delta_rule_bwd_dhu(
    ...
    def kernel(
        ...
        h0: T.Tensor(h0_shape, dtype=state_dtype),
        ...
        with T.Kernel(T.ceildiv(DV, block_DV), B * H, threads=threads) as (bv, bbh):
            ...
            if use_final_state_gradient:
                T.copy(dht[bb, bh, 0:DK, bv * block_DV:(bv + 1) * block_DV], b_dh_fragment)
            else:
...
def run_test(
    ...
    jit_kernel = tilelang_chunk_gated_delta_rule_bwd_dhu(B, S, H, DK, DV, input_dtype, output_dtype, accum_dtype, gate_dtype, state_dtype, chunk_size, scale, use_g, use_initial_state, use_final_state_gradient, block_DV, threads, num_stages)
    dh_tilelang, dh0_tilelang, dv2_tilelang = jit_kernel(Q, K, W, G, h0, dht, dO, dv)

    fla_time = do_bench(chunk_gated_delta_rule_bwd_dhu, Q, K, W, G, h0, dht, dO, dv, scale, chunk_size=chunk_size)
    tilelang_time = do_bench(jit_kernel, Q, K, W, G, h0, dht, dO, dv)

    print(f"fla time: {fla_time} ms")
    print(f"tilelang time: {tilelang_time} ms")

    assert_similar(dh_tilelang, dh_ref, name="dh", raise_assert=False)
    assert_similar(dh0_tilelang, dh0_ref, name="dh0", raise_assert=False)
    assert_similar(dv2_tilelang, dv2_ref, name="dv2", raise_assert=False)

    test_result(dh_ref, dh0_ref, dv2_ref, dh_tilelang, dh0_tilelang, dv2_tilelang, "fla-tilelang")
    ...
    if use_torch:
        ...
        assert_similar(dh_ref_torch, dh_ref, name="dh", raise_assert=False)
        assert_similar(dh0_ref_torch, dh0_ref, name="dh0", raise_assert=False)
        assert_similar(dv2_ref_torch, dv2_ref, name="dv2", raise_assert=False)
        assert_similar(dh_ref_torch, dh_tilelang, name="dh", raise_assert=False)
        assert_similar(dh0_ref_torch, dh0_tilelang, name="dh0", raise_assert=False)
        assert_similar(dv2_ref_torch, dv2_tilelang, name="dv2", raise_assert=False)

        test_result(dh_ref_torch, dh0_ref_torch, dv2_ref_torch, dh_ref, dh0_ref, dv2_ref, "torch-fla")
        test_result(dh_ref_torch, dh0_ref_torch, dv2_ref_torch, dh_tilelang, dh0_tilelang, dv2_tilelang, "torch-tilelang")
